...
route(Departing, Arriving, Visited, DepartureTime, ArrivalTime, Flights, TotalCost) :-
  not(is_visited(Visited, Departing)),

  flight(Departing, Arriving, DepartureTime, ArrivalTime, TotalCost),
  Flights = [Departing, Arriving].

route(Departing, Arriving, Visited, DepartureTime, ArrivalTime, Flights, TotalCost) :-
  flight(Departing, Immediate, X, Y, InitCost),
  not(is_visited(Visited, Departing)),
  is_less_than(X, Y),
  route(Immediate, Arriving, [Departing|[Arriving|Visited]], Q, Z, RestFlights, RestCost),
  is_less_than(Y, Q),
  is_less_than(Q, Z), 
  DepartureTime = X,
  ArrivalTime = Z,
